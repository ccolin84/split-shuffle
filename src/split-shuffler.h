...
private:
  ...
  size_t get_number_of_temp_files(const size_t) const;
  std::string get_temp_file_name(const size_t) const;
  void set_temp_file_names(const size_t);
  std::vector<std::ofstream> get_temp_file_ofstreams() const;
  void place_nums_in_random_temp_files() const;
  void write_shuffled_chunks_to_outfile();
  size_t fill_buffer_from_file(const std::string &);
